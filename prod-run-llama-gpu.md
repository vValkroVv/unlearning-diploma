# Production Llama GPU Runs

This runbook runs the 19 requested methods on `Llama-3.1-8B-Instruct`, one method at a
time.
...
Llama uses one model family for all phases. DUET uses the Llama DUET SFT
subfolder, while RWKU uses the Llama base model path directly through the
campaign wrapper.
...
DualCF variant and AdaPop in the Llama output tree.
...
AdaPop is artifact-free. It uses the same Llama `OUTPUT_ROOT` as the other
methods: `/data/home/vkropoti/unlearning/saves/unlearn`.
...
Run these after all Llama jobs finish. This runbook uses the original Llama
production root, `/data/home/vkropoti/unlearning/saves/unlearn`, so pass
`${OUTPUT_ROOT}` directly to packaging.
...
bash package_saves.sh \
  --path_to_saves "${OUTPUT_ROOT}" \
  --out_path /data/home/vkropoti/unlearning/saves-clean-llama \
  --save_eval 0
```
