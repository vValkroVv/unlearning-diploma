...
bash scripts/dualcf/run_campaign_one_lr.sh "${GPU_ID}" 1e-4 all
```
# Original H100 operator command log

This file preserves the exact machine-specific command history from the diploma
production runbook. Use `prod-run-dual-gpu.md` and `scripts/env/example.env` for
new public reproductions; paths in this file are historical examples only.
