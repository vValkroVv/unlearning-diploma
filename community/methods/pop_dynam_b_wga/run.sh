...
if [[ "${use_sft_base}" == "1" ]]; then
    ...
    echo "[pop_dynam_b_wga] Using locally finetuned base checkpoint at ${base_model_path}"
else
    base_model_path="${hf_base_model_path}"
    echo "[pop_dynam_b_wga] Using Hugging Face base checkpoint ${base_model_path}"
fi

# Trainer and experiment preset
trainer="PopDynamBWGA"
...
lrs=(1e-5 2e-5 4e-5 6e-5 8e-5 1e-4) #1e-5 2e-5 4e-5 6e-5
gammas=(1.0)
...
for split in "${forget_retain_splits[@]}"; do
    ...
    case "${forget_split}" in
        ...
        city_forget_rare_5)
            ...
            ;;
        *)
            echo "[pop_dynam_b_wga] Unknown forget split '${forget_split}', defaulting alpha=1.0"
            alpha=1.0
            ;;
    esac
            ...
            for lora_r in "${lora_rs[@]}"; do
                for lora_alpha in "${lora_alphas[@]}"; do
                    for lora_dropout in "${lora_dropouts[@]}"; do
                            ...
                            task_name=duet_${base_model}_${forget_split}_pop_dynam_b_wga_lora_r${lora_r}_lalpha${lora_alpha}_ldrop${dropout_tag}_lr${lr}_alpha${alpha_tag}_gamma${gamma_tag}
                            run_dir=${repo_root}/saves/unlearn/pop_dynam_b_wga/${task_name}
                            eval_dir=${run_dir}/evals
                            summary_path=${eval_dir}/DUET_SUMMARY.json

                            if [[ -f "${summary_path}" && "${FORCE_RERUN:-0}" != "1" ]]; then
                                echo "[pop_dynam_b_wga] Skipping ${task_name}: found existing summary at ${summary_path}"
                                continue
                            fi

                            echo "${task_name}: LoRA pop_dynam_b_wga unlearning ${base_model_path} on ${forget_split} (alpha=${alpha})"
                            ...
                            if [[ ! -f "${adapter_path}" || "${FORCE_RERUN:-0}" == "1" ]]; then
                                ...
                                echo "[pop_dynam_b_wga] Adapter already exists at ${adapter_path}; skipping training."
                            fi
