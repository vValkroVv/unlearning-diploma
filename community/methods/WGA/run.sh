...
if [[ "${use_sft_base}" == "1" ]]; then
    ...
    echo "[WGA] Using locally finetuned base checkpoint at ${base_model_path}"
else
    base_model_path="${hf_base_model_path}"
    echo "[WGA] Using Hugging Face base checkpoint ${base_model_path}"
fi

trainers_experiments=("WGA unlearn/duet/wga_lora.yaml")
...
# 8e-5 1e-4 3e-4
lrs=(1e-5 2e-5 4e-5 6e-5) #  8e-5 1e-4;;; 1e-6 5e-5 1e-5 2e-5 4e-5 5e-5 ;; 1e-5 2e-5 4e-5 6e-5
alphas=(0.5)
betas=(5.0 ) #1.0 0.5 0.1 2.0 1.0 0.1 0.01
gammas=(1.0)
                        ...
                        for lora_r in "${lora_rs[@]}"; do
                            for lora_alpha in "${lora_alphas[@]}"; do
                                for lora_dropout in "${lora_dropouts[@]}"; do
                                    ...
                                    task_name=duet_${base_model}_${forget_split}_${trainer}_lora_r${lora_r}_lalpha${lora_alpha}_ldrop${dropout_tag}_lr${lr}_beta${beta}_alpha${method_alpha_tag}_gamma${gamma_tag}
                                    # Save under a special folder for WGA runs
                                    run_dir=${repo_root}/saves/unlearn/wga/${task_name}
                                    ...
                                    echo "${task_name}: LoRA WGA unlearning ${base_model_path} on ${forget_split}"
                                    ...
                                    python src/train.py --config-name=unlearn.yaml \
                                        ...
                                        echo "[WGA] Adapter already exists at ${adapter_path}; skipping training."
                                    fi
