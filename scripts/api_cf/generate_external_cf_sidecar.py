...
def log(message: str) -> None:
    print(f"[generate_external_cf_sidecar] {message}", flush=True)
...
def build_openai_messages(
    *,
    ...
    for idx, rule in enumerate(build_prompt_rules(prompt_family), start=1):
        system_lines.append(f"{idx}. {rule}")
    system_lines.extend(
        [
            "Return JSON only.",
            "JSON schema expectations:",
            "- alternates: list of short incorrect alternatives",
            "- scores: per-candidate overall quality in [0,1]",
            "- relation_scores: per-candidate relation-preservation score in [0,1]",
            "- shared_fact_scores: per-candidate shared-fact-preservation score in [0,1]",
            "- candidate_sources: per-candidate provenance strings",
            "- answer_type: short label like year/date/int/short_span/free",
        ]
    )
    ...
    user_lines = [
        ...
        f"Return exactly {num_alternates} candidates if possible.",
    ]
    if candidate_answers:
        user_lines.append("Candidate bank hints:")
        for idx, candidate in enumerate(candidate_answers, start=1):
            user_lines.append(f"{idx}. {clean_text(candidate)}")
...
def build_sidecar_row(
    *,
    ...
    return {
        ...
        "prompt_version": f"{args.backend}:{args.prompt_family}:v1",
...
def openai_parse_bundle(
    *,
    ...
    completion_api = getattr(getattr(client, "chat", None), "completions", None)
    if completion_api is not None and hasattr(completion_api, "parse"):
        response = completion_api.parse(
            model=model,
            messages=messages,
            response_format=CandidateBundle,
        ...
        if parsed is None:
            raise RuntimeError("chat.completions.parse returned no parsed payload")
        return parsed
    ...
    if beta_completions is not None and hasattr(beta_completions, "parse"):
        response = beta_completions.parse(
            model=model,
            messages=messages,
            response_format=CandidateBundle,
            temperature=temperature,
        ...
        parsed = response.choices[0].message.parsed
        if parsed is None:
            raise RuntimeError("beta.chat.completions.parse returned no parsed payload")
        return parsed
    ...
    if responses_api is not None and hasattr(responses_api, "parse"):
        ...
        if parsed is None:
            raise RuntimeError("responses.parse returned no parsed payload")
        return parsed

    raise RuntimeError(
        "This openai-python build does not expose a structured parsing helper. "
        "Upgrade the SDK or use the codex_cli backend."
    )


def generate_openai_sidecar(
    *,
    args: argparse.Namespace,
    ...
    try:
        ...
        raise RuntimeError("openai-python is required for --backend openai_api") from exc
    ...
    client = OpenAI(
        ...
        max_retries=max(0, int(args.max_attempts) - 1),
    )
    default_source = f"openai_api:{args.model}"
    written = 0
    ...
    with output_path.open(mode, encoding="utf-8") as fout:
        for row_no, row in enumerate(rows, start=1):
            ...
            if written % 25 == 0:
                log(f"openai rows_written={written}")
...
def generate_codex_sidecar(
    *,
    ...
    with output_path.open(mode, encoding="utf-8") as fout:
        for batch_id, start in enumerate(range(0, len(pending_rows), args.batch_size), start=1):
...
def main() -> None:
    ...
    if args.backend == "openai_api":
        written = generate_openai_sidecar(
            args=args,
            rows=rows,
