...
experiment="unlearn/rwku/grad_ascent_lora.yaml"
trainer="GradAscent"

output_root="${OUTPUT_ROOT:-${repo_root}/saves/unlearn/rwku/ga}"
mkdir -p "${output_root}"
    ...
    for lora_r in "${lora_rs[@]}"; do
        for lora_alpha in "${lora_alphas[@]}"; do
            for lora_dropout in "${lora_dropouts[@]}"; do
                ...
                task_name=rwku_${base_model}_${forget_split}_ga_lora_r${lora_r}_lalpha${lora_alpha}_ldrop${dropout_tag}_lr${lr}
                ...
                if [[ -f "${summary_path}" && "${FORCE_RERUN:-0}" != "1" ]]; then
                    echo "[rwku][GA] Skipping ${task_name}: found existing summary at ${summary_path}"
                    continue
                fi
                ...
                if [[ "${delete_model_safetensors_after_eval}" == "1" ]]; then
                    if compgen -G "${run_dir}/*.safetensors" > /dev/null; then
                        ...
                        echo "[rwku][GA] Removed safetensors from ${run_dir}"
                    fi
                fi
            done
        done
