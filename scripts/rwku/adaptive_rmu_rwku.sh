...
echo "[rwku][AdaptiveRMU] Using Hugging Face base checkpoint ${base_model_path}"

experiment="${EXPERIMENT:-unlearn/rwku/adaptive_rmu_lora.yaml}"
trainer="${TRAINER:-AdaptiveRMU}"

output_root="${OUTPUT_ROOT:-${repo_root}/saves/unlearn/rwku/adaptive_rmu}"
mkdir -p "${output_root}"
                    ...
                    for lora_r in "${lora_rs[@]}"; do
                        for lora_alpha in "${lora_alphas[@]}"; do
                            for lora_dropout in "${lora_dropouts[@]}"; do
                                ...
                                task_name=rwku_${base_model}_${forget_split}_adaptive_rmu_lora_r${lora_r}_lalpha${lora_alpha}_ldrop${dropout_tag}_lr${lr}_alpha${alpha_tag}_gamma${gamma_tag}_steer${steering_tag}_scale${scale_tag}
                                ...
                                if [[ -f "${summary_path}" && "${FORCE_RERUN:-0}" != "1" ]]; then
                                    echo "[rwku][AdaptiveRMU] Skipping ${task_name}: found existing summary at ${summary_path}"
                                    continue
                                fi
                                ...
                                if [[ "${delete_model_safetensors_after_eval}" == "1" ]]; then
                                    if compgen -G "${run_dir}/*.safetensors" > /dev/null; then
                                        ...
                                        echo "[rwku][AdaptiveRMU] Removed safetensors from ${run_dir}"
                                    fi
                                fi
                            done
                        done
