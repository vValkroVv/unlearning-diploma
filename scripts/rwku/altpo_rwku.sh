...
if [[ -z "${CF_DATASET_DATA_FILES:-}" ]]; then
  export CF_DATASET_DATA_FILES="${ALTPO_ARTIFACT_ROOT}/rwku/llama31_8b_level2/altpo_forget_level2_alt${ALTPO_REPEATS}_seed${ALTPO_ARTIFACT_SEED_EFFECTIVE}.jsonl"
fi

if [[ ! -f "${CF_DATASET_DATA_FILES}" ]]; then
  echo "[rwku][AltPO] missing generated AltPO artifact: ${CF_DATASET_DATA_FILES}" >&2
  echo "Run: SEEDS='${ALTPO_ARTIFACT_SEED_EFFECTIVE}' bash scripts/altpo/prepare_altpo_artifacts.sh rwku" >&2
  exit 1
fi
