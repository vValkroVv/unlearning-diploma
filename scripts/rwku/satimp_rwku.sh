...
echo "[rwku][SatImp] Using Hugging Face base checkpoint ${base_model_path}"

experiment="unlearn/rwku/satimp_lora.yaml"
trainer="SatImp"

output_root="${OUTPUT_ROOT:-${repo_root}/saves/unlearn/rwku/satimp}"
mkdir -p "${output_root}"
                    ...
                    for lora_r in "${lora_rs[@]}"; do
                        for lora_alpha in "${lora_alphas[@]}"; do
                            for lora_dropout in "${lora_dropouts[@]}"; do
                                ...
                                task_name=rwku_${base_model}_${forget_split}_satimp_lora_r${lora_r}_lalpha${lora_alpha}_ldrop${dropout_tag}_lr${lr}_beta1${beta1_tag}_beta2${beta2_tag}_alpha${alpha_tag}_gamma${gamma_tag}
                                ...
                                if [[ -f "${summary_path}" && "${FORCE_RERUN:-0}" != "1" ]]; then
                                    echo "[rwku][SatImp] Skipping ${task_name}: found existing summary at ${summary_path}"
                                    continue
                                fi
                                ...
                                if [[ "${delete_model_safetensors_after_eval}" == "1" ]]; then
                                    if compgen -G "${run_dir}/*.safetensors" > /dev/null; then
                                        ...
                                        echo "[rwku][SatImp] Removed safetensors from ${run_dir}"
                                    fi
                                fi
                            done
                        done
