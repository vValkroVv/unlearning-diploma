...
echo "[rwku][ada_WGD] Using Hugging Face base checkpoint ${base_model_path}"

experiment="unlearn/rwku/wga_lora.yaml"
trainer="AdaWGD"

output_root="${repo_root}/saves/unlearn/rwku/ada_WGD"
mkdir -p "${output_root}"
                ...
                for lora_r in "${lora_rs[@]}"; do
                    for lora_alpha in "${lora_alphas[@]}"; do
                        for lora_dropout in "${lora_dropouts[@]}"; do
                            ...
                            task_name=rwku_${base_model}_${forget_split}_ada_WGD_lora_r${lora_r}_lalpha${lora_alpha}_ldrop${dropout_tag}_lr${lr}_${atag}_${btag}_gamma${gamma_tag}
                            run_dir=${output_root}/${task_name}
                            eval_dir=${run_dir}/evals
                            summary_path=${eval_dir}/DUET_SUMMARY.json

                            if [[ -f "${summary_path}" && "${FORCE_RERUN:-0}" != "1" ]]; then
                                echo "[rwku][ada_WGD] Skipping ${task_name}: found existing summary at ${summary_path}"
                                continue
                            fi
