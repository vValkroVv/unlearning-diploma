...
echo "[rwku][WGA] Using Hugging Face base checkpoint ${base_model_path}"

experiment="unlearn/rwku/wga_lora.yaml"
trainer="WGA"

output_root="${OUTPUT_ROOT:-${repo_root}/saves/unlearn/rwku/wga}"
mkdir -p "${output_root}"
                ...
                for lora_r in "${lora_rs[@]}"; do
                    for lora_alpha in "${lora_alphas[@]}"; do
                        for lora_dropout in "${lora_dropouts[@]}"; do
                            ...
                            task_name=rwku_${base_model}_${forget_split}_wga_lora_r${lora_r}_lalpha${lora_alpha}_ldrop${dropout_tag}_lr${lr}_beta${beta_tag}_alpha${alpha_tag}_gamma${gamma_tag}
                            ...
                            if [[ -f "${summary_path}" && "${FORCE_RERUN:-0}" != "1" ]]; then
                                echo "[rwku][WGA] Skipping ${task_name}: found existing summary at ${summary_path}"
                                continue
                            fi
                            ...
                            if [[ "${delete_model_safetensors_after_eval}" == "1" ]]; then
                                if compgen -G "${run_dir}/*.safetensors" > /dev/null; then
                                    ...
                                    echo "[rwku][WGA] Removed safetensors from ${run_dir}"
                                fi
                            fi
                        done
                    done
