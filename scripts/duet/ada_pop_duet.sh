...
if [[ "${use_sft_base}" == "1" ]]; then
    ...
    echo "[duet][AdaPop] Using locally finetuned base checkpoint at ${base_model_path}"
else
    base_model_path="${hf_base_model_path}"
    default_tokenizer_model_path="${hf_base_model_path}"
    echo "[duet][AdaPop] Using Hugging Face base checkpoint ${base_model_path}"
fi
                    ...
                    for lora_r in "${lora_rs[@]}"; do
                        for lora_alpha in "${lora_alphas[@]}"; do
                            for lora_dropout in "${lora_dropouts[@]}"; do
                                ...
                                task_name=duet_${base_model}_${forget_label}_ada_pop_lora_r${lora_r}_lalpha${lora_alpha}_ldrop${dropout_tag}_lr${lr}_${atag}_${btag}${beta_a_tag}${beta_b_tag}_gamma${gamma_tag}
                                ...
                                if [[ -f "${summary_path}" && "${FORCE_RERUN:-0}" != "1" ]]; then
                                    echo "[duet][AdaPop] Skipping ${task_name}: found existing summary at ${summary_path}"
                                    continue
                                fi
                                ...
                                if [[ "${delete_model_safetensors_after_eval}" == "1" ]]; then
                                    if compgen -G "${run_dir}/*.safetensors" > /dev/null; then
                                        ...
                                        echo "[duet][AdaPop] Removed safetensors from ${run_dir}"
                                    fi
                                fi
                            done
                        done
