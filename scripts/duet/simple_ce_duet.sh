...
if [[ "${use_sft_base}" == "1" ]]; then
    ...
    echo "[duet][SimpleCE] Using locally finetuned base checkpoint at ${base_model_path}"
else
    base_model_path="${hf_base_model_path}"
    default_tokenizer_model_path="${hf_base_model_path}"
    echo "[duet][SimpleCE] Using Hugging Face base checkpoint ${base_model_path}"
fi
...
experiment="${EXPERIMENT:-unlearn/duet/simple_ce_lora.yaml}"
trainer="SimpleCE"
...
if [[ "${cf_dataset_path}" == "json" ]] && is_nullish "${cf_dataset_data_files}"; then
    echo "[duet][${run_label}] ERROR: Local JSON mode requires CF_DATASET_DATA_FILES=/abs/path/to/duet_simple_ce.jsonl"
    exit 1
fi
                    ...
                    for lora_r in "${lora_rs[@]}"; do
                        for lora_alpha in "${lora_alphas[@]}"; do
                            for lora_dropout in "${lora_dropouts[@]}"; do
                                ...
                                task_name=duet_${base_model}_${forget_label}_${method_name}_lora_r${lora_r}_lalpha${lora_alpha}_ldrop${dropout_tag}_lr${lr}_cf${cf_weight_tag}_ret${retain_weight_tag}_gamma${gamma_tag}
