...
if [[ "${use_sft_base}" == "1" ]]; then
    ...
    echo "[duet][SatImp] Using locally finetuned base checkpoint at ${base_model_path}"
else
    base_model_path="${hf_base_model_path}"
    default_tokenizer_model_path="${hf_base_model_path}"
    echo "[duet][SatImp] Using Hugging Face base checkpoint ${base_model_path}"
fi
...
experiment="unlearn/duet/satimp_lora.yaml"
trainer="SatImp"

output_root="${OUTPUT_ROOT:-${repo_root}/saves/unlearn/duet/satimp}"
mkdir -p "${output_root}"
...
raw_beta1s="${SATIMP_BETA1S:-5.0}"
raw_beta1s="${raw_beta1s//,/ }"
raw_beta1s="${raw_beta1s//\"/}"
raw_beta1s="${raw_beta1s//\'/}"
read -r -a beta1s <<< "${raw_beta1s}"

raw_beta2s="${SATIMP_BETA2S:-0.1}"
raw_beta2s="${raw_beta2s//,/ }"
raw_beta2s="${raw_beta2s//\"/}"
raw_beta2s="${raw_beta2s//\'/}"
read -r -a beta2s <<< "${raw_beta2s}"

raw_alphas="${ALPHAS:-0.1}"
                        ...
                        for lora_r in "${lora_rs[@]}"; do
                            for lora_alpha in "${lora_alphas[@]}"; do
                                for lora_dropout in "${lora_dropouts[@]}"; do
                                    ...
                                    task_name=duet_${base_model}_${forget_label}_satimp_lora_r${lora_r}_lalpha${lora_alpha}_ldrop${dropout_tag}_lr${lr}_beta1${beta1_tag}_beta2${beta2_tag}_alpha${alpha_tag}_gamma${gamma_tag}
                                    ...
                                    if [[ -f "${summary_path}" && "${FORCE_RERUN:-0}" != "1" ]]; then
                                        echo "[duet][SatImp] Skipping ${task_name}: found existing summary at ${summary_path}"
                                        continue
                                    fi

                                    echo "${task_name}: SatImp LoRA unlearning ${base_model_path} on ${forget_split}"
                                    ...
                                    if [[ "${delete_model_safetensors_after_eval}" == "1" ]]; then
                                        if compgen -G "${run_dir}/*.safetensors" > /dev/null; then
                                            ...
                                            echo "[duet][SatImp] Removed safetensors from ${run_dir}"
                                        fi
                                    fi
                                done
                            done
