...
if [[ "${use_sft_base}" == "1" ]]; then
    ...
    echo "[duet][RMU] Using locally finetuned base checkpoint at ${base_model_path}"
else
    base_model_path="${hf_base_model_path}"
    default_tokenizer_model_path="${hf_base_model_path}"
    echo "[duet][RMU] Using Hugging Face base checkpoint ${base_model_path}"
fi
...
experiment="${EXPERIMENT:-unlearn/duet/rmu_lora.yaml}"
trainer="${TRAINER:-RMU}"

output_root="${OUTPUT_ROOT:-${repo_root}/saves/unlearn/duet/rmu}"
mkdir -p "${output_root}"
...
raw_steering_coeffs="${RMU_STEERING_COEFFS:-${STEERING_COEFFS:-2.0}}"
raw_steering_coeffs="${raw_steering_coeffs//,/ }"
raw_steering_coeffs="${raw_steering_coeffs//\"/}"
raw_steering_coeffs="${raw_steering_coeffs//\'/}"
read -r -a steering_coeffs <<< "${raw_steering_coeffs}"

raw_alphas="${RMU_ALPHAS:-${ALPHAS:-1.0}}"
raw_alphas="${raw_alphas//,/ }"
raw_alphas="${raw_alphas//\"/}"
raw_alphas="${raw_alphas//\'/}"
read -r -a alphas <<< "${raw_alphas}"

raw_gammas="${RMU_GAMMAS:-${GAMMAS:-1.0}}"
raw_gammas="${raw_gammas//,/ }"
raw_gammas="${raw_gammas//\"/}"
raw_gammas="${raw_gammas//\'/}"
read -r -a gammas <<< "${raw_gammas}"

rmu_retain_loss_type="${RMU_RETAIN_LOSS_TYPE:-EMBED_DIFF}"
rmu_module_regex="${RMU_MODULE_REGEX:-.*layers\\.7$}"
rmu_trainable_params_regex="${RMU_TRAINABLE_PARAMS_REGEX:-.*lora_[AB].*}"
                    ...
                    for lora_r in "${lora_rs[@]}"; do
                        for lora_alpha in "${lora_alphas[@]}"; do
                            for lora_dropout in "${lora_dropouts[@]}"; do
                                ...
                                task_name=duet_${base_model}_${forget_label}_rmu_lora_r${lora_r}_lalpha${lora_alpha}_ldrop${dropout_tag}_lr${lr}_steer${steering_tag}_alpha${alpha_tag}_gamma${gamma_tag}
                                ...
                                if [[ -f "${summary_path}" && "${FORCE_RERUN:-0}" != "1" ]]; then
                                    echo "[duet][RMU] Skipping ${task_name}: found existing summary at ${summary_path}"
                                    continue
                                fi

                                echo "${task_name}: RMU LoRA unlearning ${base_model_path} on ${forget_split}"
                                ...
                                if [[ ! -f "${adapter_path}" || "${FORCE_RERUN:-0}" == "1" ]]; then
                                    ...
                                    train_cmd=(
                                        ...
                                        trainer.method_args.retain_loss_type=${rmu_retain_loss_type}
                                        "trainer.method_args.module_regex='${rmu_module_regex}'"
                                        "trainer.method_args.trainable_params_regex=['${rmu_trainable_params_regex}']"
                                ...
                                if [[ "${delete_model_safetensors_after_eval}" == "1" ]]; then
                                    if compgen -G "${run_dir}/*.safetensors" > /dev/null; then
                                        ...
                                        echo "[duet][RMU] Removed safetensors from ${run_dir}"
                                    fi
                                fi
                            done
                        done
