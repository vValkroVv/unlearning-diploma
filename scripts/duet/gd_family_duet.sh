...
if [[ -n "${FORGET_SPLIT_OVERRIDE:-}" && -n "${RETAIN_SPLIT_OVERRIDE:-}" ]]; then
  ...
  forget_retain_splits=("${FORGET_SPLIT_OVERRIDE} ${RETAIN_SPLIT_OVERRIDE} ${override_label}")
fi
                  ...
                  for pdu_step in "${pdu_step_values[@]}"; do
                    for tpo_beta in "${tpo_beta_values[@]}"; do
                      for tpo_pl in "${tpo_pl_values[@]}"; do
                        ...
                        task_name=duet_${base_model}_${forget_label}_${method_name}_lora_r${lora_r}_lalpha${lora_alpha}_ldrop${dropout_tag}_lr${lr}_${method_suffix}
