...
if [[ "${use_sft_base}" == "1" ]]; then
    ...
    echo "[duet][AdaptiveRMU] Using locally finetuned base checkpoint at ${base_model_path}"
else
    base_model_path="${hf_base_model_path}"
    default_tokenizer_model_path="${hf_base_model_path}"
    echo "[duet][AdaptiveRMU] Using Hugging Face base checkpoint ${base_model_path}"
fi
...
experiment="${EXPERIMENT:-unlearn/duet/adaptive_rmu_lora.yaml}"
trainer="${TRAINER:-AdaptiveRMU}"

output_root="${OUTPUT_ROOT:-${repo_root}/saves/unlearn/duet/adaptive_rmu}"
mkdir -p "${output_root}"
...
raw_alphas="${ADAPTIVE_RMU_ALPHAS:-${ALPHAS:-1200.0}}"
raw_alphas="${raw_alphas//,/ }"
raw_alphas="${raw_alphas//\"/}"
raw_alphas="${raw_alphas//\'/}"
read -r -a alphas <<< "${raw_alphas}"

raw_gammas="${ADAPTIVE_RMU_GAMMAS:-${GAMMAS:-1.0}}"
raw_gammas="${raw_gammas//,/ }"
raw_gammas="${raw_gammas//\"/}"
raw_gammas="${raw_gammas//\'/}"
read -r -a gammas <<< "${raw_gammas}"

raw_steering_coeffs="${ADAPTIVE_RMU_STEERING_COEFFS:-1.0}"
raw_steering_coeffs="${raw_steering_coeffs//,/ }"
raw_steering_coeffs="${raw_steering_coeffs//\"/}"
raw_steering_coeffs="${raw_steering_coeffs//\'/}"
read -r -a steering_coeffs <<< "${raw_steering_coeffs}"

raw_scales="${ADAPTIVE_RMU_SCALES:-5.0}"
raw_scales="${raw_scales//,/ }"
raw_scales="${raw_scales//\"/}"
raw_scales="${raw_scales//\'/}"
read -r -a adaptive_scales <<< "${raw_scales}"

adaptive_rmu_coeff_mode="${ADAPTIVE_RMU_COEFF_MODE:-first_batch}"
adaptive_rmu_module_regex="${ADAPTIVE_RMU_MODULE_REGEX:-.*layers\\.7$}"
adaptive_rmu_trainable_params_regex="${ADAPTIVE_RMU_TRAINABLE_PARAMS_REGEX:-.*model\\.layers\\.(5|6|7)\\..*lora_[AB].*}"
    ...
    for lr in "${lrs[@]}"; do
        for alpha in "${alphas[@]}"; do
            ...
            for gamma in "${gammas[@]}"; do
                gamma_tag=${gamma//./p}
                for steering_coeff in "${steering_coeffs[@]}"; do
                    steering_tag=${steering_coeff//./p}
                    for adaptive_scale in "${adaptive_scales[@]}"; do
                        scale_tag=${adaptive_scale//./p}
                        for lora_r in "${lora_rs[@]}"; do
                            for lora_alpha in "${lora_alphas[@]}"; do
                                for lora_dropout in "${lora_dropouts[@]}"; do
                                    dropout_tag=${lora_dropout//./p}
                                    task_name=duet_${base_model}_${forget_label}_adaptive_rmu_lora_r${lora_r}_lalpha${lora_alpha}_ldrop${dropout_tag}_lr${lr}_alpha${alpha_tag}_gamma${gamma_tag}_steer${steering_tag}_scale${scale_tag}
                                    ...
                                    if [[ -f "${summary_path}" && "${FORCE_RERUN:-0}" != "1" ]]; then
                                        echo "[duet][AdaptiveRMU] Skipping ${task_name}: found existing summary at ${summary_path}"
                                        continue
                                    fi

                                    echo "${task_name}: AdaptiveRMU LoRA unlearning ${base_model_path} on ${forget_split}"
                                    ...
                                    if [[ ! -f "${adapter_path}" || "${FORCE_RERUN:-0}" == "1" ]]; then
                                        ...
                                        train_cmd=(
                                            ...
                                            trainer.method_args.adaptive_scale=${adaptive_scale}
                                            trainer.method_args.adaptive_coeff_mode=${adaptive_rmu_coeff_mode}
                                            "trainer.method_args.module_regex='${adaptive_rmu_module_regex}'"
                                            "trainer.method_args.trainable_params_regex=['${adaptive_rmu_trainable_params_regex}']"
                                    ...
                                    if [[ "${delete_model_safetensors_after_eval}" == "1" ]]; then
                                        if compgen -G "${run_dir}/*.safetensors" > /dev/null; then
                                            ...
                                            echo "[duet][AdaptiveRMU] Removed safetensors from ${run_dir}"
                                        fi
                                    fi
                                done
                            done
