...
if [[ "${use_sft_base}" == "1" ]]; then
    ...
    echo "[duet][UNDIAL] Using locally finetuned base checkpoint at ${base_model_path}"
else
    base_model_path="${hf_base_model_path}"
    default_tokenizer_model_path="${hf_base_model_path}"
    echo "[duet][UNDIAL] Using Hugging Face base checkpoint ${base_model_path}"
fi
...
experiment="${EXPERIMENT:-unlearn/duet/undial_lora.yaml}"
trainer="${TRAINER:-UNDIAL}"

output_root="${OUTPUT_ROOT:-${repo_root}/saves/unlearn/duet/undial}"
mkdir -p "${output_root}"
                    ...
                    for lora_r in "${lora_rs[@]}"; do
                        for lora_alpha in "${lora_alphas[@]}"; do
                            for lora_dropout in "${lora_dropouts[@]}"; do
                                ...
                                task_name=duet_${base_model}_${forget_label}_undial_lora_r${lora_r}_lalpha${lora_alpha}_ldrop${dropout_tag}_lr${lr}_beta${beta_tag}_alpha${alpha_tag}_gamma${gamma_tag}
                                ...
                                if [[ -f "${summary_path}" && "${FORCE_RERUN:-0}" != "1" ]]; then
                                    echo "[duet][UNDIAL] Skipping ${task_name}: found existing summary at ${summary_path}"
                                    continue
                                fi
                                ...
                                if [[ "${delete_model_safetensors_after_eval}" == "1" ]]; then
                                    if compgen -G "${run_dir}/*.safetensors" > /dev/null; then
                                        ...
                                        echo "[duet][UNDIAL] Removed safetensors from ${run_dir}"
                                    fi
                                fi
                            done
                        done
