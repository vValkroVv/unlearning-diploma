...
if [[ "${use_sft_base}" == "1" ]]; then
    ...
    echo "[duet][STAT] Using locally finetuned base checkpoint at ${base_model_path}"
else
    base_model_path="${hf_base_model_path}"
    default_tokenizer_model_path="${hf_base_model_path}"
    echo "[duet][STAT] Using Hugging Face base checkpoint ${base_model_path}"
fi
...
experiment="unlearn/duet/stat_lora.yaml"
trainer="STAT"

output_root="${OUTPUT_ROOT:-${repo_root}/saves/unlearn/duet/stat}"
mkdir -p "${output_root}"
                            ...
                            for lora_r in "${lora_rs[@]}"; do
                                for lora_alpha in "${lora_alphas[@]}"; do
                                    for lora_dropout in "${lora_dropouts[@]}"; do
                                        ...
                                        task_name=duet_${base_model}_${forget_label}_stat_lora_r${lora_r}_lalpha${lora_alpha}_ldrop${dropout_tag}_lr${lr}_sfw${sfw_tag}_srw${srw_tag}_mode${stat_mode}_xsp${xsp_tag}_peos${peos_tag}
                                        ...
                                        if [[ -f "${summary_path}" && "${FORCE_RERUN:-0}" != "1" ]]; then
                                            echo "[duet][STAT] Skipping ${task_name}: found existing summary at ${summary_path}"
                                            continue
                                        fi
                                        ...
                                        if [[ "${delete_model_safetensors_after_eval}" == "1" ]]; then
                                            if compgen -G "${run_dir}/*.safetensors" > /dev/null; then
                                                ...
                                                echo "[duet][STAT] Removed safetensors from ${run_dir}"
                                            fi
                                        fi
                                    done
                                done
