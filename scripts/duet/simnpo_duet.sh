...
if [[ "${use_sft_base}" == "1" ]]; then
    ...
    echo "[duet][SimNPO] Using locally finetuned base checkpoint at ${base_model_path}"
else
    base_model_path="${hf_base_model_path}"
    default_tokenizer_model_path="${hf_base_model_path}"
    echo "[duet][SimNPO] Using Hugging Face base checkpoint ${base_model_path}"
fi
...
experiment="unlearn/duet/simnpo_lora.yaml"
trainer="SimNPO"

output_root="${OUTPUT_ROOT:-${repo_root}/saves/unlearn/duet/simnpo}"
mkdir -p "${output_root}"
...
raw_deltas="${DELTAS:-0.0}"
raw_deltas="${raw_deltas//,/ }"
raw_deltas="${raw_deltas//\"/}"
raw_deltas="${raw_deltas//\'/}"
read -r -a deltas <<< "${raw_deltas}"
                        ...
                        for lora_r in "${lora_rs[@]}"; do
                            for lora_alpha in "${lora_alphas[@]}"; do
                                for lora_dropout in "${lora_dropouts[@]}"; do
                                    ...
                                    task_name=duet_${base_model}_${forget_label}_simnpo_lora_r${lora_r}_lalpha${lora_alpha}_ldrop${dropout_tag}_lr${lr}_delta${delta_tag}_beta${beta_tag}_alpha${alpha_tag}_gamma${gamma_tag}
                                    ...
                                    if [[ -f "${summary_path}" && "${FORCE_RERUN:-0}" != "1" ]]; then
                                        echo "[duet][SimNPO] Skipping ${task_name}: found existing summary at ${summary_path}"
                                        continue
                                    fi

                                    echo "${task_name}: SimNPO LoRA unlearning ${base_model_path} on ${forget_split}"
                                    ...
                                    if [[ "${delete_model_safetensors_after_eval}" == "1" ]]; then
                                        if compgen -G "${run_dir}/*.safetensors" > /dev/null; then
                                            ...
                                            echo "[duet][SimNPO] Removed safetensors from ${run_dir}"
                                        fi
                                    fi
                                done
                            done
