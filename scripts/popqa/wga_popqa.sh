...
if [[ "${use_sft_base}" == "1" ]]; then
    ...
    echo "[popqa][WGA] Using locally finetuned base checkpoint at ${base_model_path}"
else
    base_model_path="${hf_base_model_path}"
    echo "[popqa][WGA] Using Hugging Face base checkpoint ${base_model_path}"
fi

experiment="unlearn/popqa/wga_lora.yaml"
trainer="WGA"

output_root="${repo_root}/saves/unlearn/popqa/wga"
mkdir -p "${output_root}"
                    ...
                    for lora_r in "${lora_rs[@]}"; do
                        for lora_alpha in "${lora_alphas[@]}"; do
                            for lora_dropout in "${lora_dropouts[@]}"; do
                                ...
                                task_name=popqa_${base_model}_${forget_split}_wga_lora_r${lora_r}_lalpha${lora_alpha}_ldrop${dropout_tag}_lr${lr}_beta${beta_tag}_alpha${alpha_tag}_gamma${gamma_tag}
                                run_dir=${output_root}/${task_name}
                                eval_dir=${run_dir}/evals
                                summary_path=${eval_dir}/POPQA_SUMMARY.json

                                if [[ -f "${summary_path}" && "${FORCE_RERUN:-0}" != "1" ]]; then
                                    echo "[popqa][WGA] Skipping ${task_name}: found existing summary at ${summary_path}"
                                    continue
                                fi
