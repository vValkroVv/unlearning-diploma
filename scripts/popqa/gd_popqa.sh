...
if [[ "${use_sft_base}" == "1" ]]; then
    ...
    echo "[popqa][GD] Using locally finetuned base checkpoint at ${base_model_path}"
else
    base_model_path="${hf_base_model_path}"
    echo "[popqa][GD] Using Hugging Face base checkpoint ${base_model_path}"
fi

experiment="unlearn/popqa/grad_diff_lora.yaml"
trainer="GradDiff"

output_root="${repo_root}/saves/unlearn/popqa/gd"
mkdir -p "${output_root}"
