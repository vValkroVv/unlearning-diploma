...
models=(
  ...
  "gemma-7b-it|gemma-7b-it-lora|/mnt/extremessd10tb/borisiuk/open-unlearning/saves/finetune/popqa/gemma-7b-it_full_5ep_ft_popqa"
  "Qwen2.5-7B-Instruct|Qwen2.5-7B-Instruct-lora|/mnt/extremessd10tb/borisiuk/open-unlearning/saves/finetune/popqa/qwen2.5-7b_full_5ep_ft_popqa"
)

for model_row in "${models[@]}"; do
  IFS='|' read -r base_model model_cfg base_path <<< "${model_row}"
  echo "[metrics][popqa] model=${base_model}"

  for algo_dir in "${repo_root}/saves/unlearn/popqa"/*; do
    [ -d "${algo_dir}" ] || continue
